...
<div class="container mx-auto flex justify-center items-center">
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width={width} height={height} style="background-color:dimgray"> 
        ...
        <g fill="white" stroke="black">
            {#each californiaMap.features as feature, i}
                <path d={geoGenerator(feature)} on:click={() => selected = feature} class="county" in:draw={{ delay: i * 50, duration: 1000 }}/>
            {/each}
        </g>
        ...
        {#each points as [cx,cy]}
            <circle cx={cx} cy={cy} r="1" stroke="black" fill="black" />
        {/each}
    </svg>
</div>
